...
class DoctorModel(models.Model):
    ...
    user = models.OneToOneField('Accounts.User', on_delete=models.CASCADE, related_name='doctor_profile', null=True,
                                blank=True)
    department = models.ForeignKey('Department.DepartmentModel', models.SET_NULL, 'department_doctors', null=True,blank=True)
...
class EducationDetailsModel(models.Model):
    ...
    # if cls.country:
    #     university_choices = [(uni['name'], uni['name']) for uni in universities if
    #                           uni.get('country') == cls.country]
    #     print('hiii')

    # if country:
    #     university_choices = [(uni['name'], uni['name']) for uni in universities if uni.get('country').lower() == country.lower().strip()]
    #     print(len(university_choices))
    # else:
    #     university_choices = [(uni['name'], uni['name']) for uni in universities if uni.get('country') == 'Iran']
    #     print(len(university_choices))
    ...
    #     selected_country = cleaned_data.get('country')
