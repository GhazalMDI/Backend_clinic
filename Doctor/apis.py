...
class PatientAPIView(APIView):
    def get(self, request, doctor_id):
        if doctor := DoctorModel.objects.filter(pk=doctor_id).first():
            appointment = AppointmentModel.objects.filter(doctor_id=doctor)
        ...
        return get_Response(
            success=False,
            status=400,
            message='دکتری یافت نشد یا دسترسی ندارید!'
        )
